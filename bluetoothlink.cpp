...
BluetoothLink* BluetoothLink::m_instance = nullptr;
BluetoothConfiguration* BluetoothConfiguration::m_instance = nullptr;

BluetoothLink::BluetoothLink(QObject *perent):
      _connectState(false)
    , _targetSocket(nullptr)
    ,_config(BluetoothConfiguration::getInstance())
#ifdef __ios__
...
void BluetoothLink::_writeBytes(const QByteArray bytes)
{
    if (_targetSocket) {
        if(_targetSocket->write(bytes) > 0) {
            ...
            qDebug() << "Bluetooth write error";
        }
    }
}

void BluetoothLink::_writeBytes(const char *data, qint64 len)
{
    if (_targetSocket) {
        if(_targetSocket->write(data,len) > 0) {
            //_logOutputDataRate(bytes.size(), QDateTime::currentMSecsSinceEpoch());
        } else {
            qDebug() << "Bluetooth write error";
        }
...
{
    qDebug()<<"Bluetooth disconnect called";
#ifdef __ios__
    if(_discoveryAgent) {
        _shutDown = true;
        _discoveryAgent->stop();
...
void BluetoothLink::serviceDiscovered(const QBluetoothServiceInfo& info)
{
    ...
    D libNewUIAeroGCS_armeabi-v7a.so: Bluetooth Device currIndex :  1
    D libNewUIAeroGCS_armeabi-v7a.so:
}
#endif
...
BluetoothConfiguration::BluetoothConfiguration(QObject *parent)
...
{
    BluetoothLink *ulink = BluetoothLink::getInstance();
//    if(_link)
    {
//        BluetoothLink* ulink = dynamic_cast<BluetoothLink*>();
        if(ulink) {
...
{
    qDebug() << "Bluetooth Device name : "<<name;
...
void BluetoothConfiguration::setDevName(const int currIndex)
{
    qDebug() << "Bluetooth Device currIndex : "<<currIndex;
    if(_nameList.count()>0)
    {
        QString name = _nameList.at(currIndex);
        for(const BluetoothData& data: _deviceList)
        {
            if(data.name == name)
            {
...
QString BluetoothConfiguration::getDeviceNameAt(int m_index)
{
    if( (_nameList.count() > m_index) && (_nameList.count() > 0) )
    {
        return _nameList.at(m_index);
    }
    return "";
}
