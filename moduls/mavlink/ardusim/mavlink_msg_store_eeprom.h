...
#define MAVLINK_MSG_ID_STORE_EEPROM_CRC 83
#define MAVLINK_MSG_ID_5_CRC 83
...
#define MAVLINK_MESSAGE_INFO_STORE_EEPROM { \
    ...
    {  { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 2, offsetof(mavlink_store_eeprom_t, target_system) }, \
         ...
         { "sendACK", NULL, MAVLINK_TYPE_UINT8_T, 0, 4, offsetof(mavlink_store_eeprom_t, sendACK) }, \
...
static inline uint16_t mavlink_msg_store_eeprom_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, uint8_t sendACK, uint16_t address, uint8_t len, const uint8_t *buffer)
{
...
{
    return mavlink_msg_store_eeprom_pack(system_id, component_id, msg, store_eeprom->target_system, store_eeprom->target_component, store_eeprom->sendACK, store_eeprom->address, store_eeprom->len, store_eeprom->buffer);
}
...
{
    return mavlink_msg_store_eeprom_pack_chan(system_id, component_id, chan, msg, store_eeprom->target_system, store_eeprom->target_component, store_eeprom->sendACK, store_eeprom->address, store_eeprom->len, store_eeprom->buffer);
}
...
static inline void mavlink_msg_store_eeprom_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint8_t sendACK, uint16_t address, uint8_t len, const uint8_t *buffer)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_STORE_EEPROM_LEN];
    _mav_put_uint16_t(buf, 0, address);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_store_eeprom_send(chan, store_eeprom->target_system, store_eeprom->target_component, store_eeprom->sendACK, store_eeprom->address, store_eeprom->len, store_eeprom->buffer);
#else
 ...
 */
static inline void mavlink_msg_store_eeprom_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_system, uint8_t target_component, uint8_t sendACK, uint16_t address, uint8_t len, const uint8_t *buffer)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field sendACK from store_eeprom message
 *
 * @return  operation response.
 */
static inline uint8_t mavlink_msg_store_eeprom_get_sendACK(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_store_eeprom_decode(const mavlink_message_t* msg, mavlink_store_eeprom_t* store_eeprom)
{
    ...
    store_eeprom->sendACK = mavlink_msg_store_eeprom_get_sendACK(msg);
